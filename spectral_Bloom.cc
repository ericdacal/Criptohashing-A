...
spectral_Bloom::spectral_Bloom(int size, int numHash, int numKeys) {
...
int spectral_Bloom::hash1(long long value) {
...
int spectral_Bloom::hash2(long long value) {
...
void spectral_Bloom::addKey (const string& key) {
...
bool spectral_Bloom::queryKey(const string& key) {
    long long representation = transformStringToInt(key);
    for (int i = 0;i < hashFunctionsNum;++i) {
    	int indexH1 = hash1(representation);
    	int indexH2 = hash2(representation);
...
void spectral_Bloom::remove(const string& key) {
    long long representation = transformStringToInt(key);
    for (int i = 0;i < hashFunctionsNum;++i) {
    	int indexH1 = hash1(representation);
    	int indexH2 = hash2(representation);
...
int spectral_Bloom::num_occurs(const string& key)
{
